...
DATA_FILE = './data/exp_3_pos.db'
data_x_pos = np.genfromtxt(DATA_FILE, delimiter=' ')
DATA_FILE = './data/exp_3_neg.db'
data_x_neg=  np.genfromtxt(DATA_FILE, delimiter=' ')


pca=PCA(n_components=10)
data_x_pos=pca.fit_transform(data_x_pos)
pca=PCA(n_components=10)
data_x_neg=pca.fit_transform(data_x_neg)
f= open("./data/exp_3T_pos.db","w+")
for row in data_x_pos:
    for feat in row:
        f.write(str(feat)+" ")
    f.write("\n")
f.close()
f= open("./data/exp_3T_neg.db","w+")
...
data_x_pos,data_x_neg, names = new_pred.pred_creation('./data/exp_3T_pos.db','./data/exp_3T_neg.db',2,trig_func=False,only_init=True,inequalities=True)
data_x = np.concatenate((data_x_pos,data_x_neg), axis=0)
n_components=data_x.shape[1]
...
for index,val in  enumerate (names):
    predColl.add_continous(name=val,no_lt=6,no_gt=6)

for i in range(1):
    predColl.add_pred(name='class_%d'%(i+1),arguments=[] , variables=[] , pFunc = DNF('class_%d'%(i+1),terms=4,init=[-1,.1,-1,.1],sig=2), use_cnt_vars=True,inc_preds=[
])
...
parser.add_argument('--ITER', default=200*60, help='Maximum number of iteration',type=int)
